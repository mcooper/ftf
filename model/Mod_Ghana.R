...
lc <- read.csv('Landcover.csv') %>%
  unique %>%
  mutate(pop=pop/5625,  #Convert to 1000pp/sqkm
         market=market/24) %>% #Convert to days
...
all <- Reduce(merge, list(spi, lc, hh, ltn)) %>%
  select(hhs, spi6, spi12, spi24, spi36, asset_index, pop, market, hh_size, male_5de, female_5de, combo_5de, latitude, longitude, hh_refno,
         hhhead_religion, hhhead_age, hhhead_education, hhhead_literate, hhhead_sex, dependents, urban_rural, mean_annual_precip) %>%
...
# }
# 
# all <- rescale_many(all, c("spi24", "asset_index", "pop", "market", "hh_size", 
...
#LM
lm1 <- lm(hhs~asset_index + dependents + hhhead_education + hhhead_age + 
            hhhead_literate + hhhead_religion + hh_size + hhhead_sex +
            pop + market + spi24 + mean_annual_precip, 
          data=filter(all, urban_rural=="Rural"))
summary(lm1)


lm2 <- lm(hhs~asset_index + dependents + hhhead_education + hhhead_age + 
            hhhead_literate + hhhead_religion + hh_size + hhhead_sex +
            pop + market + urban_rural + spi24 + mean_annual_precip, 
          data=all)
summary(lm2)
...
#SPLM
splm1 <- errorsarlm(hhs~asset_index + dependents + hhhead_education + hhhead_age +
                      hhhead_literate + hhhead_religion + hh_size + hhhead_sex +
                      pop + market + spi24 + mean_annual_precip + spi24*mean_annual_precip,
                    data=all, wlisthh)
summary(splm1)


write.csv(summary(splm1)$Coef %>% as.data.frame,
          'G://My Drive/Papers/Feed the Future/Ghana_Effects.csv')


calchhs <- function(spi, precip){
  hhs <- 7.0192 + -4.089*spi + 0.3625*precip + -0.584*spi*precip
  hhs
}

spi <- seq(0, 2, length.out=50)
precip <- seq(9, 14, 1)

df <- expand.grid(spi, precip)
names(df) <- c('spi', 'precip')

df$HHS <- mapply(FUN=calchhs, spi=df$spi, precip=df$precip)

df$precip <- factor(df$precip)

ggplot(df) + geom_line(aes(x=spi, y=HHS, color=precip)) +
  geom_point(data=all, aes(x=spi24, y=hhs))
